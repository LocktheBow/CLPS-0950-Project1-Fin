function enhancedRunTrainingPhase(window, tasks, screenInfo)
% ENHANCEDRUNTRAININGPHASE - Enhanced training module with proper key detection
%
% This function provides examples and explanations for each task type,
% with improved key detection and escape key functionality.
%
% Inputs:
...
% Record function start for logging
fprintf('Starting enhanced training phase...\n');
...
% Clear any previous keypresses and wait for a new one
FlushEvents('keyDown');
[~, ~, keyCode] = KbStrokeWait;

% Check for escape key press using safe method
if safeCheckEscape(keyCode)
    fprintf('Training phase exited by user request.\n');
    return;
end

WaitSecs(0.5);

% Show dedicated training examples for similarities (separated from main question bank)
for i = 1:length(tasks.similarities.trainingPairs)
    % Get the current training pair
    ...
    % Use the training options and correct indices
    options = tasks.similarities.trainingOptions{i};
    correctIdx = tasks.similarities.trainingCorrectIndices(i);
    ...
    % Add escape key option
    DrawFormattedText(window, 'Press any key to continue...', 'center', screenInfo.screenYpixels * 0.85, screenInfo.black);
    DrawFormattedText(window, 'Press ESC to exit', 'center', screenInfo.screenYpixels * 0.9, [0.8, 0, 0]);
    Screen('Flip', window);
    
    % Get key press but ONLY check for escape, ignore number keys
    [~, ~, keyCode] = KbStrokeWait;
    
    % Check for escape key using safe method
    if safeCheckEscape(keyCode)
        fprintf('Training phase exited by user request.\n');
        return;
    end
    
    % Print what key was pressed for debugging
    try
        pressedKey = KbName(keyCode);
        fprintf('Key pressed during training: %s\n', pressedKey);
    catch
        fprintf('Could not determine pressed key name.\n');
    end
...
% Clear keypresses and wait for a new one
FlushEvents('keyDown');
[~, ~, keyCode] = KbStrokeWait;
...
% Use questionTracker to select and mark examples as used
if exist('questionTracker', 'file') == 2
    try
        % Find unused vocabulary examples
        fprintf('Finding unused vocabulary questions for training...\n');
        allIndices = 1:length(tasks.vocabulary);
        availableIndices = [];
        
        % Check which indices are still available
        for i = 1:length(allIndices)
            try
                if questionTracker('check', 'vocabulary', allIndices(i))
                    availableIndices = [availableIndices, allIndices(i)];
                end
            catch
        ...
        % If we have enough unused examples, select 3 random ones
        if length(availableIndices) >= 3
            randOrder = randperm(length(availableIndices));
            exampleIndices = availableIndices(randOrder(1:3));
        else
            % Fall back to random selection if not enough unused
            exampleIndices = randi(length(tasks.vocabulary), 1, 3);
        end
        
        % Mark these as used
        questionTracker('mark', 'vocabulary', exampleIndices);
        fprintf('Selected vocabulary examples: %s\n', mat2str(exampleIndices));
    catch e
...
if exist('questionTracker', 'file') == 2
    try
        % Find unused information examples
        fprintf('Finding unused information questions for training...\n');
        ...
        for i = 1:length(allIndices)
            try
                if questionTracker('check', 'information', allIndices(i))
                    availableIndices = [availableIndices, allIndices(i)];
                end
            catch
                % Skip this index if error
        ...
        if length(availableIndices) >= 3
            ...
            % Fall back to random selection if not enough unused
            exampleIndices = randi(length(tasks.information), 1, 3);
        end
        
        % Mark these as used
        questionTracker('mark', 'information', exampleIndices);
        fprintf('Selected information examples: %s\n', mat2str(exampleIndices));
    catch e
        fprintf('Error with question tracking: %s\n', e.message);
        exampleIndices = randi(length(tasks.information), 1, 3);
    end
...
DrawFormattedText(window, 'Press any key to continue to practice...', 'center', screenInfo.screenYpixels * 0.85, screenInfo.black);
DrawFormattedText(window, 'Press ESC to exit', 'center', screenInfo.screenYpixels * 0.9, [0.8, 0, 0]);
Screen('Flip', window);
...
% No need to mark training examples as used in the question tracker anymore,
% since they're separate from the main question bank

fprintf('Enhanced training phase completed successfully.\n');
WaitSecs(0.5);
end

function isEscapePressed = safeCheckEscape(keyCode)
% SAFECHECKESCAPE - Helper function to safely check for escape key press
%
% This handles both scalar and array key indices that KbName might return
% and provides better error handling.

    % Use the improved checkEscapeKey function if available
    if exist('checkEscapeKey', 'file') == 2
        try
            isEscapePressed = checkEscapeKey(keyCode);
            return;
        catch e
            fprintf('Error using checkEscapeKey from safeCheckEscape: %s\n', e.message);
            % Continue with built-in method
        end
    end
    ...
    try
        ...
        % Debug output
        if isEscapePressed
            fprintf('Escape key detected by safeCheckEscape function\n');
        end
    catch e
        % In case of any error, try alternate methods
        fprintf('Warning: Error in primary escape key check: %s\n', e.message);
        
        try
            % Try alternate methods
            isEscapePressed = any(find(keyCode) == 27); % ASCII for Escape
            
            if ~isEscapePressed
                % Try multiple escape key possibilities
                escKeys = [27, 41, 53, 54]; % Common escape key codes
                isEscapePressed = any(ismember(find(keyCode), escKeys));
            end
        catch
            % Ultimate fallback
            fprintf('Warning: All escape key detection methods failed.\n');
